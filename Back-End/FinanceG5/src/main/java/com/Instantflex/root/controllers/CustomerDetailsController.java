...
//	}
	...
	public void addCustomer(@RequestBody CustomerDetails c) {
		...
		if(c.getCardtype().equals("gold")) {
			cc.setCreditbalance(200000);
			cc.setCredittotal(200000);
		}
		else {
			cc.setCreditbalance(1000000);
			cc.setCredittotal(1000000);
		}
		cc.setCardvalidity(new Date("2026/12/12"));
		c.setCarddetails(cc);
	...
	@GetMapping("/afterUserById/{id}")
	public CustomerDetails afterCustomerById(@PathVariable("id") int id) {
		return service.getCustomerById(id);
	}
	...
	public CustomerDetails validateCustomer(@RequestParam("username") String username, @RequestParam("password") String password) {
		System.out.println("Value of username and passsword"+username+" ""+password);
